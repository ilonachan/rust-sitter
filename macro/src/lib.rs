...
/// 
/// TODO: what about named precedence?
/// TODO: it makes sense that left and right can't be combined, but what about dynamic precedence? Should that be separate?
///
/// ## Example
/// ```ignore
...
/// Marks a list of tokens which could appear in a conflict.
/// TODO: How are these tokens referenced?
/// That is easy for structs and enums, but what about inner leafs represented only by properties?
/// It's possible that conflicts like this can only arise at enums representing choices, because the
/// way our grammar tree can be built might be slightly more limited than usual... that would be
/// perfect, because it would mean conflicts can be marked directly on the enum!
/// They do have to be marked, because this system is opt-in and automatically marking the whole
/// enum as a conflict situation could create massive runtime overhead (from trying all options at once)
#[proc_macro]
pub fn known_conflict(_item: proc_macro::TokenStream) -> proc_macro::TokenStream {
    proc_macro::TokenStream::new()
}

/// Marks a relative ordering for a list of precedence labels.
/// TODO: how to represent these?
/// This could either be strings (as tree-sitter obviously does it), but that's not very safe.
/// Alternatively we could force the user to make an Enum and use that.
/// Or the user simply writes precedence names as identifiers wherever they're needed,
/// an enum is autogenerated, and only those items can be used in this annotation!
#[proc_macro]
pub fn precedence(_item: proc_macro::TokenStream) -> proc_macro::TokenStream {
    proc_macro::TokenStream::new()
}
